...
global iterout imp_mat imms isgs irstr ipgorder lim cfl Cx Cy toler rkappa Re pinf uinf rho rhoinv xmin xmax ymin ymax Cx2 Cy2 fsmall
...
rhist_var=50; %var for how often reshist is stored
[nvec,artn,resvec,convVec]=res_hist_init(nmax,rhist_var);
...
for n = ninit:nmax
    ...
    %% store & plot res/conv history 
    % updates plotting data every 50 itt, updates plot every 500 itt
    if ( (mod(n,rhist_var)==0)||(n==ninit))
        [artn,nvec,resvec,convVec]=res_hist_vecs(n,conv,res,artn,nvec,resvec,convVec);
    end
    % plots residuals and conv (updates plot every 500 itt)
    if ( (mod(n,500)==0)||(n==ninit) )
    figure(1)
    plot(nvec(1:artn,:),resvec(1:artn,:))
    ylim([0,1])
    set(gca, 'YScale', 'log')
    figure(2)
    plot(nvec(1:artn,:),convVec(1:artn,:),'color','b')
    ylim([0,1])
    set(gca, 'YScale', 'log')
    end
...
%**************************************************************************
% Initalizes residual and conv history vectors
function [nvec,artn,resvec,convVec]=res_hist_init(nmax,rhist_var)
%--- Varibles used in res hist function 
artmax=1+ceil(nmax/rhist_var);     %length of history vectors 
nvec=zeros(artmax,1);              % vector that stores the ittoration number
artn=0;                          % artifical n (artifical collection itteration number)
resvec=zeros(artmax,3);            % residual history vector
convVec=zeros(artmax,1);           % convergance history vector
end



%**************************************************************************
% Stores residual and conv histories
function [artn,nvec,resvec,convVec] = res_hist_vecs(n,conv,res,artn,nvec,resvec,convVec)
    artn=artn+1;
    nvec(artn)=n;
    resvec(artn,:)=res';
    convVec(artn)=conv;
    ...
    for i=2:imax-1
        ...
        if i==2||i==imax-1||j==2||j==jmax-1
                    if i==2
                        ...
                        d4pdx4 = (three*u(i,j,1)-frtn*u(i+1,j,1)+twntsx*u(i+2,j,1)...
                -twntfr*u(i+3,j,1)+elevn*u(i+4,j,1)-two*u(i+5,j,1))/(dx^4);
                        d4pdy4 = (three*u(i,j,1)-frtn*u(i,j+1,1)+twntsx*u(i,j+2,1)...
                -twntfr*u(i,j+3,1)+elevn*u(i,j+4,1)-two*u(i,j+5,1))/(dy^4);
                        elseif j==jmax-1                                    % top left corner 
                        d4pdx4 = (three*u(i,j,1)-frtn*u(i+1,j,1)+twntsx*u(i+2,j,1)...
                -twntfr*u(i+3,j,1)+elevn*u(i+4,j,1)-two*u(i+5,j,1))/(dx^4);
                        d4pdy4 = (three*u(i,j,1)-frtn*u(i,j-1,1)+twntsx*u(i,j-2,1)...
                -twntfr*u(i,j-3,1)+elevn*u(i,j-4,1)-two*u(i,j-5,1))/(dy^4);
                        ...
                        d4pdx4 = (three*u(i,j,1)-frtn*u(i+1,j,1)+twntsx*u(i+2,j,1)...
                -twntfr*u(i+3,j,1)+elevn*u(i+4,j,1)-two*u(i+5,j,1))/(dx^4);
                        d4pdy4 = (u(i,j+2,1)-four*u(i,j+1,1)+six*u(i,j,1)-four*u(i,j-1,1)...
            +u(i,j-2,3))/(dy^4); 
                        end
                    end
                    if i==imax-1 
                ...
                -twntfr*u(i-3,j,1)+elevn*u(i-4,j,1)-two*u(i-5,j,1))/(dx^4);
                        d4pdy4 = (three*u(i,j,1)-frtn*u(i,j+1,1)+twntsx*u(i,j+2,1)...
                -twntfr*u(i,j+3,1)+elevn*u(i,j+4,1)-two*u(i,j+5,1))/(dy^4);
                        elseif j==jmax-1                                   % top right corner
                        d4pdx4 = (three*u(i,j,1)-frtn*u(i-1,j,1)+twntsx*u(i-2,j,1)...
                -twntfr*u(i-3,j,1)+elevn*u(i-4,j,1)-two*u(i-5,j,1))/(dx^4);
                        d4pdy4 = (three*u(i,j,1)-frtn*u(i,j-1,1)+twntsx*u(i,j-2,1)...
                -twntfr*u(i,j-3,1)+elevn*u(i,j-4,1)-two*u(i,j-5,1))/(dy^4);
            ...
            +u(i,j-2,1))/(dy^4); 
                    ...
                    if i~=2 && i~=imax-1
                        ...
                        d4pdx4 = (u(i+2,j,1)-four*u(i+1,j,1)+six*u(i,j,1)-four*u(i-1,j,1)...
            +u(i-2,j,1))/(dx^4);
                        d4pdy4 = (three*u(i,j,1)-frtn*u(i,j+1,1)+twntsx*u(i,j+2,1)...
                -twntfr*u(i,j+3,1)+elevn*u(i,j+4,1)-two*u(i,j+5,1))/(dy^4);
                        end
            ...
            +u(i-2,j,1))/(dx^4);
                ...
                -twntfr*u(i,j-3,1)+elevn*u(i,j-4,1)-two*u(i,j-5,1))/(dy^4);
                                                                            ...
                                                                            % interior points
            d4pdx4 = (u(i+2,j,1)-four*u(i+1,j,1)+six*u(i,j,1)-four*u(i-1,j,1)...
            +u(i-2,j,1))/(dx^4);                                           % 4th derivative of pressure w.r.t. x
        
            d4pdy4 = (u(i,j+2,1)-four*u(i,j+1,1)+six*u(i,j,1)-four*u(i,j-1,1)...
            +u(i,j-2,1))/(dy^4);                                           % 4th derivative of pressure w.r.t. y
        end
...
% !************************************************************** */
% !************Think that this might need to change...************ */
% !************************************************************** */
R=zeros(imax,jmax,neq);
for j=1:jmax
    for i=1:imax
        for k=1:neq
            R(i,j,k)=(u(i,j,k)-uold(i,j,k))/dt(i,j);      
        end
    end
end
